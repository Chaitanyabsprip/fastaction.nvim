...
M.get_key = function(name, key_not_valid, key_valid)
    ...
    repeat
        ...
        if char:match("[a-z]") and not vim.tbl_contains(key_not_valid, char) then return char end
        index = index + 1
    until index >= #name

    for _, k in pairs(key_valid) do if not vim.tbl_contains(key_not_valid, k) then return k end end
    return nil
end
